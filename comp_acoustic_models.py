"""
This script implements three acoustic models in their vanilla form without putting an RNN layer on top.

How to use this code
1. Download the datasets and generate spectrograms.
2. Create a folder, e.g., maps/dense if you want to train the dense acoustic model on the MAPS dataset, and copy this
   script to the folder. By default, the checkpoints will be saved in folder dense/saved_model, and the statistics and
   other information will be saved in folder dense/tb_d0. You can view the outputs with tensorboard.
3. Configure the following parameters:
    ...
    ACOUSTIC_MODEL: in {'dense', 'harmonic', 'dilated'}. This sets the acoustic model you want to run.
...
class MiscFns(object):
    ...
    @staticmethod
    def label_fn(sr, spec_stride, mid_file_name, num_frames):
        sr = int(sr)
        spec_stride = int(spec_stride)
        assert (sr, spec_stride) in [(16000, 512), (44100, 22 * 64)]
    ...
    @staticmethod
    def harmonic_or_dilated_acoustic_model_fn(mode, spec_batch, is_training, use_feature, trainable):

        assert tf.get_variable_scope().name != ''

        assert mode in ('harmonic', 'dilated')
        is_harmonic_mode = mode == 'harmonic'
        ...
        if is_harmonic_mode:
            ...
            outputs.set_shape([None, None, 88 * 3, num_features])
        else:
            outputs = slim.conv2d(
                scope='DC_4',
                inputs=outputs,
        ...
        outputs.set_shape([None, None, 88, num_features])
    ...
    @staticmethod
    def dense_acoustic_model_fn(big_or_small, spec_batch, is_training, use_feature, trainable):

        assert tf.get_variable_scope().name != ''

        assert big_or_small in ('big', 'small')
        use_more_features = big_or_small == 'big'
        ...
        outputs = slim.conv2d(
            ...
            inputs=outputs,
            kernel_size=3,
            num_outputs=48 if use_more_features else 32,
            normalizer_fn=slim.batch_norm,
            normalizer_params=dict(is_training=is_training, trainable=trainable),
            trainable=trainable
        )
        ...
        outputs = slim.max_pool2d(scope='MP_1', inputs=outputs, stride=[1, 2], kernel_size=[1, 2], padding='VALID')
        outputs = slim.dropout(scope='DO_1', is_training=is_training, inputs=outputs, keep_prob=.75)

        outputs = slim.conv2d(
            scope='C_2',
            inputs=outputs,
            kernel_size=3,
            num_outputs=96 if use_more_features else 64,
            normalizer_fn=slim.batch_norm,
            normalizer_params=dict(is_training=is_training, trainable=trainable),
            trainable=trainable
        )
        outputs = slim.max_pool2d(scope='MP_2', inputs=outputs, stride=[1, 2], kernel_size=[1, 2], padding='VALID')
        outputs = slim.dropout(scope='DO_2', is_training=is_training, inputs=outputs, keep_prob=.75)

        num_features = 768 if use_more_features else 512
        f = outputs.shape[2].value
        c = outputs.shape[3].value
        assert f is not None and c is not None
        b, t, _, _ = tf.unstack(tf.shape(outputs))
        outputs = tf.reshape(outputs, [b, t, f * c])
        outputs = slim.fully_connected(
            scope='FC_3',
            inputs=outputs,
            num_outputs=num_features,
        ...
        outputs.set_shape([None, None, num_features])
        outputs = slim.dropout(scope='DO_3', is_training=is_training, inputs=outputs, keep_prob=.5)
    ...
    @staticmethod
    def num_frames_of_mel_sg_from_wav_file(wav_file):
        wav_info = soundfile.info(wav_file)
        old_sr = wav_info.samplerate
        assert old_sr in (44100, 48000)
        new_sr = 16000
        num_samples = wav_info.frames
        num_samples = (num_samples * new_sr + old_sr - 1) // old_sr
        num_frames = (num_samples + 511) // 512

        return num_frames

    @staticmethod
    def num_frames_of_vqt_sg_from_wav_file(wav_file):
        wav_info = soundfile.info(wav_file)
        old_sr = wav_info.samplerate
        assert old_sr in (44100, 48000)
        new_sr = 44100
    ...
    @staticmethod
    def frame_label_detector_fn(model_name, inputs, is_training, **kwargs):

        assert tf.get_variable_scope().name == ''
        assert model_name in ('dilated', 'harmonic', 'dense')
        if model_name == 'dense':
            assert kwargs['big_or_small'] in ('big', 'small')
        assert isinstance(is_training, bool)

        if model_name == 'dense':
            vs_name = 'dense_{}_frame_detector'.format(kwargs['big_or_small'])
            with tf.variable_scope(vs_name, reuse=tf.AUTO_REUSE):
                logits = MiscFns.dense_acoustic_model_fn(
                    big_or_small=kwargs['big_or_small'],
                    spec_batch=inputs,
            ...
            vs_name = '{}_frame_detector'.format(model_name)
            with tf.variable_scope(vs_name, reuse=tf.AUTO_REUSE):
                logits = MiscFns.harmonic_or_dilated_acoustic_model_fn(
                    mode=model_name,
                    spec_batch=inputs,
                    is_training=is_training,
                    use_feature=False,
    ...
    def maestro_vqt_sg_and_label_fn(year, rec_name, label_only):
        ...
        num_frames = MiscFns.num_frames_of_vqt_sg_from_wav_file(wav_file)
        ...
        sr = 44100
        mid_file = wav_file[:-3] + 'midi'
        num_frames_from_midi = mido.MidiFile(mid_file).length
        num_frames_from_midi = int(np.ceil(num_frames_from_midi * sr))
        num_frames_from_midi = (num_frames_from_midi + 63) // 64
        num_frames_from_midi = (num_frames_from_midi + 21) // 22
    ...
    @staticmethod
    def maestro_mel_sg_and_label_fn(year, rec_name, label_only):

        wav_file = os.path.join(os.environ['maestro'], year, rec_name + '.wav')
        num_frames = MiscFns.num_frames_of_mel_sg_from_wav_file(wav_file)

        if not label_only:
            rec_name = os.path.basename(wav_file)[:-4]
            mel_file = os.path.join(os.environ['maestro_mel'], year, rec_name + '.mel')
            _rec_name, mel = MiscFns.load_np_array_from_file_fn(mel_file)
            assert _rec_name == rec_name
            assert mel.shape == (num_frames, 229)
            assert mel.dtype == np.float32

        mid_file = wav_file[:-3] + 'midi'
        num_frames_from_midi = mido.MidiFile(mid_file).length
        num_frames_from_midi = int(np.ceil(num_frames_from_midi * 16000))
        num_frames_from_midi = (num_frames_from_midi + 511) // 512
        num_frames_from_midi += 2
        num_frames = min(num_frames, num_frames_from_midi)

        if not label_only:
            mel = mel[:num_frames]
            mel = np.require(mel, dtype=np.float32, requirements=['O', 'C'])
        ...
        if label_only:
            ...
            return dict(sg=mel, label=label)
    ...
    def maps_vqt_sg_and_label_fn(wav_file, label_only):
        ...
        if not label_only:
            ...
            _rec_name, vqt = MiscFns.load_np_array_from_file_fn(vqt_file)
            assert _rec_name == rec_name
            assert vqt.shape == (num_frames, 336) and vqt.dtype == np.float32

        mid_file = wav_file[:-3] + 'mid'
        num_frames_from_midi = mido.MidiFile(mid_file).length
        num_frames_from_midi = int(np.ceil(num_frames_from_midi * 44100))
        num_frames_from_midi = (num_frames_from_midi + 63) // 64
        num_frames_from_midi = (num_frames_from_midi + 21) // 22
        num_frames_from_midi += 2
        num_frames = min(num_frames, num_frames_from_midi)
    ...
    @staticmethod
    def maps_mel_sg_and_label_fn(wav_file, label_only):

        num_frames = MiscFns.num_frames_of_mel_sg_from_wav_file(wav_file)

        if not label_only:
            rec_name = os.path.basename(wav_file)[:-4]
            mel_file = os.path.join(os.environ['maps_mel'], rec_name + '.mel')
            _rec_name, mel = MiscFns.load_np_array_from_file_fn(mel_file)
            assert _rec_name == rec_name
            assert mel.shape == (num_frames, 229)
            assert mel.dtype == np.float32
...
class Config(object):
    ...
    def __init__(self):
        ...
        assert self.acoustic_model in ('dense', 'harmonic', 'dilated')
        self.acoustic_model_is_dense = self.acoustic_model == 'dense'
        if self.acoustic_model_is_dense:
            if self.training_dataset_is_maps:
                self.small_dense = True
            else:
                self.small_dense = False
        else:
            self.small_dense = None

        self.prefetch_ratio = PREFETCH_RATIO
        assert 0 < self.prefetch_ratio <= 1

        self.batches_per_epoch = 5000 if self.training_dataset_is_maps else 10000
        self.snippet_len = 1200 if self.acoustic_model == 'dense' else 600  # number of frames per example
        if self.training_dataset_is_maps:
            if self.acoustic_model == 'dense':
                self.num_epochs = 25
            elif self.acoustic_model == 'harmonic':
                self.num_epochs = 35
            elif self.acoustic_model == 'dilated':
                self.num_epochs = 35
        else:
            if self.acoustic_model == 'dense':
                self.num_epochs = 20
        ...
        if self.debug_mode:
            ...
            for tvt in self.tvt_split_dict.keys():
                ...
                _sel = np.random.choice(_num, 1, replace=False)
        ...
        # use only one recording for training and validation if run in inference mode
...
class Model(object):
    ...
    def _dataset_iter_fn(self):
        if self.is_training:
            if not self.config.acoustic_model_is_dense:
                ...
                if self.config.prefetch_ratio < .999:
                    if self.config.training_dataset_is_maps:
                        ...
                        def _read_mel_fn(rec_name):
                            mel_file = os.path.join(os.environ['maps_mel'], rec_name + '.mel')
                            _rec_name, mel = MiscFns.load_np_array_from_file_fn(mel_file)
                            assert _rec_name == rec_name
                            assert mel.shape[1:] == (229,) and mel.dtype == np.float32
                        ...
                        while True:
                            ...
                            for ord_idx, rec_idx in enumerate(shuffled_rec_idx_list):
                                ...
                                rec_name = os.path.basename(wav_file)[:-4]
                                if n_iters == 0:
                                    logging.info('{}/{} - {} - reading mel'.format(ord_idx + 1, num_recs, rec_name))
                                mel = _read_mel_fn(rec_name)
                                num_frames = num_frames_list[rec_idx]
                                assert mel.shape[0] >= num_frames
                                mel = mel[:num_frames]
                                assert mel.shape == (num_frames, 229)
                                mel = np.require(mel, dtype=np.float32, requirements=['O', 'C'])
                                mel.flags['WRITEABLE'] = False
                        ...
                        def _read_mel_fn(year, rec_name):
                            mel_file = os.path.join(os.environ['maestro_mel'], year, rec_name + '.mel')
                            _rec_name, mel = MiscFns.load_np_array_from_file_fn(mel_file)
                            assert _rec_name == rec_name
                            assert mel.dtype == np.float32 and mel.shape[1:] == (229,)
                        ...
                        while True:
                            ...
                            for ord_idx, rec_idx in enumerate(shuffled_rec_idx_list):
                                ...
                                if num_iters == 0:
                                    logging.info('{}/{} - {} - reading vqt'.format(ord_idx + 1, num_recs, rec_name))
                                mel = _read_mel_fn(year, rec_name)
                                num_frames = num_frames_list[rec_idx]
                                assert mel.shape[0] >= num_frames
                                mel = mel[:num_frames]
                                assert mel.shape == (num_frames, 229)
    ...
    def _gen_batch_fn(self):
        ...
        num_freq_bins = 229 if self.config.acoustic_model_is_dense else 336
        ...
        with tf.device('/cpu:0'):
            if self.is_training:
                dataset = tf.data.Dataset.from_generator(
                    ...
                    output_shapes=dict(spectrogram=[None, num_freq_bins], label=[None, 88], num_frames=[])
                )
    ...
    def _nn_model_fn(self):
        ...
        if self.config.acoustic_model_is_dense:
            arg_dict = dict(model_name='dense', inputs=inputs, is_training=self.is_training)
            arg_dict['big_or_small'] = 'small' if self.config.small_dense else 'big'
            outputs = _nn_fn(**arg_dict)
        else:
            outputs = _nn_fn(model_name=self.config.acoustic_model, inputs=inputs, is_training=self.is_training)
    ...
    def _gen_dataset_fn(self):
            ...
            if self.config.training_dataset_is_maps:
                for file_idx, wav_file in enumerate(file_names):
                    ...
                    if label_only:
                        ...
                        dataset.append(dict(sg=sg_label['sg'], label=sg_label['label']))
        ...
        if not self.is_training:
            ...
            assert split_name in ('validation', 'test')
            ...
            if dataset_name == 'maps':
                ...
                for file_idx, wav_file_name in enumerate(file_names):
                    ...
                    dataset.append(dict(sg=sg_label_dict['sg'], label=sg_label_dict['label']))
                self.dataset = dataset
    ...
    def _get_sg_and_label_fn(self, **kwargs):
        ...
        def _gen_sg_label_check(sg_label):
            num_freq_bins = 229 if self.config.acoustic_model_is_dense else 336
            label_only = self.is_training and self.config.prefetch_ratio < 0.999
            if label_only:
                ...
                assert sg.dtype == np.float32 and sg.shape[1:] == (num_freq_bins,)
        ...
        if not self.config.acoustic_model_is_dense:
            if self.is_training:
                if self.config.training_dataset_is_maps:
                    sg_label = MiscFns.maps_vqt_sg_and_label_fn(
                        wav_file=kwargs['wav_file'], label_only=self.config.prefetch_ratio < .999)
                else:
                    sg_label = MiscFns.maestro_vqt_sg_and_label_fn(
                ...
                if dataset_name == 'maps':
                    sg_label = MiscFns.maps_vqt_sg_and_label_fn(
                        wav_file=kwargs['wav_file'], label_only=False)
                else:
                    sg_label = MiscFns.maestro_vqt_sg_and_label_fn(
            ...
            if self.is_training:
                if self.config.training_dataset_is_maps:
                    sg_label = MiscFns.maps_mel_sg_and_label_fn(
                        wav_file=kwargs['wav_file'], label_only=self.config.prefetch_ratio < .999)
                else:
                    sg_label = MiscFns.maestro_mel_sg_and_label_fn(
                        year=kwargs['year'], rec_name=kwargs['rec_name'], label_only=self.config.prefetch_ratio < .999)
            else:
                split_name, dataset_name = self.name.split('_')
                assert split_name in ('validation', 'test') and dataset_name in ('maps', 'maestro')
                if dataset_name == 'maps':
                    sg_label = MiscFns.maps_mel_sg_and_label_fn(wav_file=kwargs['wav_file'], label_only=False)
                else:
                    sg_label = MiscFns.maestro_mel_sg_and_label_fn(
                        year=kwargs['year'], rec_name=kwargs['rec_name'], label_only=False)

        _gen_sg_label_check(sg_label)
...
def main():
    ...
    with tf.Session(config=MODEL_DICT['config'].gpu_config) as sess:
        ...
        if MODEL_DICT['config'].acoustic_model == 'dense':
            if MODEL_DICT['config'].small_dense:
                model_name = 'dense_small'
            else:
                model_name = 'dense_big'
        else:
            model_name = MODEL_DICT['config'].acoustic_model
        aug_info.append('model - {}'.format(model_name))

        training_dataset = MODEL_DICT['config'].training_dataset_is_maps
        training_dataset = 'maps' if training_dataset else 'maestro'
        aug_info.append('training dataset - {}'.format(training_dataset))
