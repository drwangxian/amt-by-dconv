"""
This script implements acoustic model HSg(1) + dense.

How to use this code
1. Download the MAPS dataset and generate the STFTs.
2. Create a folder, e.g., maps/hsg/h1, and copy this script to the folder. By default, the checkpoints will be saved in
...
class MiscFns(object):
    ...
    def frame_detector_fn(spec_batch, is_training):
        ...
        spec_batch.set_shape([None, None, 804])
        ...
        with tf.variable_scope('frame_detector', reuse=tf.AUTO_REUSE):
            ...
            outputs.set_shape([None, None, 264])
            ...
            outputs.set_shape([None, None, 512])
    ...
    def stft_to_hvqt_tf_fn(stft):
        ...
        fmin = librosa.midi_to_hz([27])[0] * 2. ** (-1. / 36)
        fmax = librosa.midi_to_hz([114])[0] * 2. ** (1. / 36)
        fmax = fmax * (1. + .04)
        f_resolution = 44100. / 5632.
        n_req_freq_bins = np.ceil(fmax / f_resolution)
        n_req_freq_bins = int(n_req_freq_bins) + 2
        assert n_req_freq_bins == 804
        f0s = fmin * 2. ** (np.arange(264, dtype=np.float32) / 36.)
        fft_bins = np.arange(n_req_freq_bins, dtype=np.float32) * f_resolution
        assert fft_bins[-1] > fmax
