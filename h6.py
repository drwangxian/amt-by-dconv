"""
This script implements acoustic model HSg(6) + dense.

How to use this code
1. Download the MAPS dataset and generate the STFTs.
2. Create a folder, e.g., maps/hsg/h6, and copy this script to the folder. By default, the checkpoints will be saved in
...
class MiscFns(object):
    ...
    def frame_detector_fn(spec_batch, is_training):
        ...
        with tf.variable_scope('frame_detector', reuse=tf.AUTO_REUSE):
            ...
            outputs.set_shape([None, None, 768])
    ...
    @staticmethod
    def stft_to_hvqt_tf_fn(stft):
        assert tf.get_variable_scope().name != ''
        stft.set_shape([None, None, 2817])

        with tf.variable_scope('bandwidth'):
            bw_alpha = tf.get_variable(
                name='alpha',
                dtype=tf.float32,
                initializer=.03851,
                trainable=True,
                constraint=lambda w: tf.maximum(w, 0.)
            )
            bw_beta = tf.get_variable(
                name='beta',
                dtype=tf.float32,
                initializer=10.,
                trainable=True,
            ...
            n_req_freq_bins = librosa.midi_to_hz([114])[0] * 2. ** (1. / 36) * 6.
            f_resolution = 44100. / 5632.
            n_req_freq_bins = np.ceil(n_req_freq_bins / f_resolution)
            n_req_freq_bins = int(n_req_freq_bins) + 2
            stft = stft[..., :-1]
            stft = tf.pad(stft, [[0, 0], [0, 0], [0, n_req_freq_bins - 2816]])

            tmp = librosa.midi_to_hz([27])[0] * 2. ** (-1. / 36)
            tmp = tmp * 2. ** (np.arange(264, dtype=np.float32) / 36.)
            f0s = []
            for h in xrange(6):
                h = h + 1
                f0s.append(tmp * h)
            f0s = np.concatenate(f0s)
            assert len(f0s) == 264 * 6
            fft_bins = np.arange(n_req_freq_bins, dtype=np.float32) * f_resolution
            assert fft_bins[-1] < 44100.
            assert fft_bins[-1] > f0s[-1]

            bw = bw_alpha * f0s + bw_beta
            bw = tf.minimum(bw, 2. * f0s)
            bw = bw[None, :]  # bw is tf tensor
            f0s = f0s[None, :]
            fft_bins = fft_bins[:, None]

            f0s = tf.constant(f0s, dtype=tf.float32)
            fft_bins = tf.constant(fft_bins, dtype=tf.float32)
            up_slope = tf.matmul(fft_bins, 2. / bw) + 1. - 2. * f0s / bw
            down_slope = tf.matmul(fft_bins, -2. / bw) + 1. + 2. * f0s / bw
            fb = tf.maximum(0., tf.minimum(up_slope, down_slope))

            stft_shape = tf.shape(stft)
            b, t, _ = tf.unstack(stft_shape)
            fb = tf.tile(fb[None, ...], [b, 1, 1])
            stft = tf.matmul(stft, fb)
            stft = tf.reshape(stft, [b, t, 6, 264])
            stft = tf.transpose(stft, [0, 1, 3, 2])
            stft.set_shape([None, None, 264, 6])
            tmp = 20. / np.log(10.)
            tmp = tf.constant(tmp, dtype=tf.float32)
            stft = tmp * tf.log(stft + tf.constant(1e-7, dtype=tf.float32)) + tf.constant(140., dtype=tf.float32)
    ...
    def _tb_summary_fn(self):
        ...
        if self.is_training:
            ...
            for var in tf.get_collection(tf.GraphKeys.TRAINABLE_VARIABLES):
                name = var.op.name
                if name.endswith('bandwidth/alpha'):
                    bw_alpha_beta_dict['alpha'] = var
                elif name.endswith('bandwidth/beta'):
                    bw_alpha_beta_dict['beta'] = var
            assert len(bw_alpha_beta_dict) == 2
            summaries = []
            with tf.name_scope('filter'):
                for name, var in bw_alpha_beta_dict.iteritems():
                    summaries.append(tf.summary.scalar(name, var))
            summaries.append(scalar_summaries)
            scalar_summaries = tf.summary.merge(summaries)
